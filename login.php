	...
	<title>LOGIN</title>
	 ...
	 <link href="css/icon.css" rel="stylesheet">
      ...
      <?php
        ...
        $sql = mysqli_query($con,"SELECT * FROM login WHERE username = '$_POST[user]' AND password = '$_POST[pass]'");
        $tampil = mysqli_fetch_array($sql);
        $cek = mysqli_num_rows($sql);
        if($cek > 0){

          echo "<script>alert('Selamat datang $_POST[user]');document.location.href='milih.php'</script>";
        }else{
  ...
  <!--JADWAL PARALLAX -->
    	<form align="center" method="post">
    		<div class="card-panel " style="width:300px; height: 250px; margin-top: 125px; margin-left: 540px;">
    			...
    			<div>
    				<input type="submit" name="login" value="login" class="waves-effect waves-light btn" style="margin-left: 100px;" id="btn4">
    			</div>
    		</div>
      </div>
    	</form>
